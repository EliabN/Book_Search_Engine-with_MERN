 ...
 * Built using the MERN stack with a React front end, MongoDB database, and Node.js/Express.js server and API.
 ...
 >**Important**: Refactoring a fully functioning Google Books API search engine built with a RESTful API to be a GraphQL API built with Apollo Server. 
...
To achieve this, the following was completed:
...
4. Deploy the application to Heroku with a MongoDB database using MongoDB Atlas. Use the [Deploy with Heroku and MongoDB Atlas](https://coding-boot-camp.github.io/full-stack/mongodb/deploy-with-heroku-and-mongodb-atlas) walkthrough for instructions.
...
The following tasks were completed in the back-end files:
...
* `Schemas` directory:
  ...
  **Important**: Used functionality in the `user-controller.js` as a guide.
...
* `queries.js`: This To hold the query `GET_ME`, which will execute the `me` query set up using Apollo Server.

* `mutations.js`:

  * `LOGIN_USER` To execute the `loginUser` mutation set up using Apollo Server.

  * `ADD_USER` To execute the `addUser` mutation.

  * `SAVE_BOOK` To execute the `saveBook` mutation.

  * `REMOVE_BOOK` To execute the `removeBook` mutation.
